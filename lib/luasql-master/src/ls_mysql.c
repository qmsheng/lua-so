...
** Set sql timeout
*/
static int conn_settimeout (lua_State *L) {
	conn_data *conn = getconnection (L);
	int args = lua_gettop(L);
	/*connect_timeout, read_timeout, write_timeout*/
	if ((!!conn) && (args == 2)){
		unsigned int out_time = luaL_checkint(L, 2);
		/*conn->my_conn->options.read_timeout = luaL_checkint(L, 2);*/
		int ret = mysql_options(conn->my_conn, MYSQL_OPT_READ_TIMEOUT, (const void *)&out_time);/*设置read超时*/
		if( ret ){
			fprintf(stderr, "Connection read is timeout!\n");
			lua_pushboolean(L, 0);
		}else{
			lua_pushboolean(L, 1);
		}
