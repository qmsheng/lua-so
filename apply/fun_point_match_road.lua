...
local function quick_location( lon, lat, dir, accountID )
	...
	local ok,old_roadID = redis_api.cmd("private", "get", accountID .. ":sysInternalRoadID")
	if not ok or not old_roadID then
		return false,nil
	end
	-- ERBR: export road by road
	local ok,array_maybe_roadID = redis_api.cmd("roadRelation", "hmget", "ERBR", old_roadID)
	...
	for _,key in ipairs( valid_road_grid_key_array ) do
		local ok, kv_tab = redis_api.cmd('mapRoadLine', 'hmget', key, array_maybe_roadID)
	...
	for idx,key in ipairs( valid_line_grid_key_array ) do
		...
		local ok, kv_tab = redis_api.cmd('mapLineNode', 'hmget', key, field_array)
	...
	if new_roadID ~= old_roadID then
		redis_api.cmd("private", "set", accountID .. ":sysInternalRoadID", new_roadID)
	end
	return true,result
end
...
local function whole_location( lon, lat, dir, accountID )
	...
	for _,key in ipairs( valid_grid_key_array ) do
		local ok, kv_tab = redis_api.cmd('mapLineNode', 'hgetall', key)
...
function detail_entry(direction, longitude, latitude, accountID, fields)
	...
	if (ok and result) and ((not fields) or (type(fields) == "table" and #fields > 0)) then
		...
		if not fields then
			...
			local ok, kv_tab = redis_api.cmd('mapRoadInfo', 'hmget', key, fields)
			if ok and kv_tab and (#kv_tab > 0) then
				for i,val in ipairs(kv_tab) do
					result[ fields[i] ] = val
				end
